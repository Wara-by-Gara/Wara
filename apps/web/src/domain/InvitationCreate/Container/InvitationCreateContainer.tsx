...
export default function InvitationCreateContainer() {
  ...
  // 병합 충돌나서 develop 브랜치에 있던 코드 잠시 보류
        ...
        <main className="flex flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
          ...
          <div className="mt-4 flex w-full max-w-xs flex-col gap-3">
            <Button
              ...
              size="lg"
              fullWidth
              onClick={() => setStep('templateCategory')}
            >
              템플릿으로 시작
            </Button>
            <Button
    ...
    return (
      <div className="relative mx-auto flex h-full min-h-svh w-full max-w-md flex-col bg-background">
        ...
        <div className="relative z-10 shrink-0">
          <StickyCTA
            primary={{ label: '다음', onClick: () => setStep('basicInfo') }}
          />
        </div>
      </div>
    );
  ...
  if (step === 'templateCategory') {
    const categories = [...new Set(templates.map((t) => t.theme))].filter(
      Boolean,
    );
    return (
      <div className="relative mx-auto flex h-full min-h-svh w-full max-w-md flex-col bg-background">
    ...
    return (
      <div className="relative mx-auto flex h-full min-h-svh w-full max-w-md flex-col bg-background">
        <TopAppBar
          className="shrink-0"
          title="템플릿"
          onBack={() => setStep('templateCategory')}
        />
              ...
              <TemplateCard
                ...
                onClick={() => {
                  if (form.templateId === t.id) {
                    ...
                    set({
                      templateId: t.id,
                      mainImageKey: t.previewImageKey ?? DEFAULT_COVER_KEY,
                    });
                    setLocalPreviewUrl(null);
                  }
                }}
    ...
    return (
      <div className="relative mx-auto flex h-full min-h-svh w-full max-w-md flex-col bg-background">
        <TopAppBar
          className="shrink-0"
          title="어떤 모임인가요?"
          onBack={() => setStep(form.templateId ? 'templateList' : 'start')}
        />
        ...
        <main className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-5 py-4">
          ...
          <FormField
            label="어떤 모임인가요?"
            required
            counter={{ current: form.title.length, max: 30 }}
            error={titleError ? '모임 이름을 입력해주세요' : undefined}
          >
            <TextInput
              value={form.title}
              onChange={(e) => {
                ...
                <section>
                  <div className="mb-2 flex items-center justify-between">
                    <p className="text-[14px] font-semibold text-text-primary">
                      선택된 미션 ({selectedMissions.length}개)
                    </p>
                    {selectedMissions.length >= MAX_MISSIONS && (
  ...
  return (
    <div
      className={cn(
        'relative mx-auto flex h-full min-h-svh w-full max-w-md flex-col',
        designBgColor,
      )}
    >
      <TopAppBar
          ...
          {form.title ? (
            <p
              className={cn(
                ...
                DESIGN_FONTS.find((f) => f.id === designFont)?.style,
              )}
            >
              {form.title}
            </p>
          ...
          <div className="grid grid-cols-5 gap-2">
            {DESIGN_BG_COLORS.map(({ cls, hex }) => (
              <button
                ...
                style={{
                  boxShadow:
                    cls === 'bg-white' ? 'inset 0 0 0 1px #e5e7eb' : undefined,
                }}
                aria-label={hex}
              />
            ))}
