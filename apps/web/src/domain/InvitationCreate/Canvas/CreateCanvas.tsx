...
import type { DesignFont, RsvpType, RsvpOption } from "@/domain/InvitationCreate/constants";
    ...
    <main className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-page pb-6 pt-4">
      ...
      <div className="relative">
        {hasCover ? (
          ...
          <button
            ...
            onClick={onEditImage}
            className={cn(
              "flex aspect-[4/5] w-full flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed",
      ...
      <div className="mt-auto grid grid-cols-3 gap-2 pt-2">
        {(["attending", "maybe", "declined"] as RsvpType[]).map((type) => (
          <div
            key={type}
            className="flex flex-col items-center gap-1.5 rounded-md border border-border/50 bg-surface/70 px-3 py-3 backdrop-blur"
          >
