...
export default function GuestView({
  ...
  const { data: myParticipant, isLoading: isLoadingMyParticipant } =
    useMyParticipant(invitationId, { enabled: isLoggedIn });
  ...
  const handleRsvp = (next: RSVPValue) => {
    ...
    if (myParticipant) {
      ...
      joinInvitation.mutate(
        { rsvpStatus: next },
        {
          onError: (err) => {
            if (
              err instanceof Error &&
              err.message === 'INVITATION_ACCESS_REVOKED'
            ) {
    ...
    >
      ...
      <div className="relative z-10 flex min-h-0 flex-1 flex-col">
          ...
          cover={
            <>
              <InvitationCover
                ...
                gifUrl={cover.gifUrl}
                backgroundClass={
                  gradientVariant ? gradientBaseCls(gradientVariant.id) : invitation.bgColor
                }
                style={
                  gradientVariant
                    ? ({
                        ['--c1']: gradientVariant.c1,
                        ['--c2']: gradientVariant.c2,
                  ...
                  <div className="mb-3">
                    ...
                    <div className="grid grid-cols-3 gap-1">
                      {Array.from({ length: 6 }).map((_, i) => (
                        <div
                          key={i}
                          className="aspect-square rounded-sm bg-border"
                        />
                      ))}
                    </div>
                  </div>
        ...
        <BottomSheet
          ...
          title="로그인이 필요해요"
          description="참석 응답을 남기려면 먼저 로그인해주세요"
        >
          <div className="flex flex-col gap-2 pt-2">
            {(['kakao', 'naver', 'google', 'apple'] as const).map(
              (provider) => (
                <SocialLoginButton
                  key={provider}
                  provider={provider}
