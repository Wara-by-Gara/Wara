...
export const Notifications = ({
        ...
        <main className="min-h-0 flex-1 overflow-y-auto">
          <section className="py-2">
            <div className="divide-y divide-border bg-surface">
              <MenuItem leftIcon="bell" rightSlot={<Switch defaultChecked />}>푸시 알림</MenuItem>
              <MenuItem leftIcon="user-check" rightSlot={<Switch defaultChecked />}>새 RSVP</MenuItem>
              <MenuItem leftIcon="message-circle" rightSlot={<Switch defaultChecked />}>새 댓글</MenuItem>
              <MenuItem leftIcon="image" rightSlot={<Switch />}>새 사진</MenuItem>
              <MenuItem leftIcon="calendar-clock" rightSlot={<Switch defaultChecked />}>모임 리마인더</MenuItem>
            </div>
          </section>
        </main>
    ...
    return (
      <div className="relative mx-auto flex h-full min-h-full w-full max-w-md flex-col overflow-x-hidden bg-background">
        ...
        <main className={mobileMainCenter}>
          <EmptyState
            icon="bell"
            title={state === "pushPermissionGuide" ? "알림을 받으시겠어요?" : "알림 권한이 꺼져있어요"}
            description={
              state === "pushPermissionGuide"
                ? "RSVP·댓글·사진 알림을 보내드릴게요"
                : "설정에서 알림을 켜면 새 소식을 받을 수 있어요"
            }
            action={<Button>{state === "pushPermissionGuide" ? "권한 허용" : "설정 열기"}</Button>}
          />
        </main>
        <MainBottomNav activeKey="notifications" />
      </div>
  ...
  return (
    <div className="relative mx-auto flex h-full min-h-full w-full max-w-md flex-col overflow-x-hidden bg-background">
      ...
      <main
        className={cn(
          state === "error" || state === "empty" ? mobileMainCenter : mobileMainScroll,
        )}
      >
        {state === "loading" ? (
