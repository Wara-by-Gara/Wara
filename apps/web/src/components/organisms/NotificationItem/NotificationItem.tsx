...
const ICON_MAP: Record<NotificationType, { icon: IconName; bg: string; color: string }> = {
  ...
  albumOpened: { icon: "retro-camera", bg: "bg-cranberry-10", color: "text-cranberry-60" },
...
export const NotificationItem = forwardRef<HTMLButtonElement, NotificationItemProps>(
  function NotificationItem(
    ...
    return (
      <div className={cn("relative flex items-start", unread && "bg-cranberry-5/60", "rounded-2xl")}>
        <button
          ref={ref}
          type="button"
          className={cn(
            "flex flex-1 items-start gap-3 rounded-2xl px-4 py-3 text-left hover:bg-gray-50 transition-colors duration-150",
